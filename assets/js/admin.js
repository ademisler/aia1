(function($){
  ...
  $(function(){ if (window.lucide && lucide.createIcons) { lucide.createIcons(); }
    ...
    if ($ls.length){ fetchLowStock($ls, 1); get(AIA.rest+'categories').then(cats=>{ if(Array.isArray(cats) && cats.length && !$('#aia-ls-cat').length){ const sel=$('<select id="aia-ls-cat" class="aia-input" style="max-width:240px;margin:10px 0;"><option value="">All categories</option></select>'); cats.forEach(c=> sel.append('<option value="'+c.slug+'">'+c.name+'</option>')); $ls.before(sel); } }); }
    ...
    // Trend chart via REST
    const c = document.getElementById('aia-chart');
    if (c && window.Chart){ get(AIA.rest+'metrics/trend').then(tr=>{ const labels=(tr&&tr.labels)||['Mon','Tue','Wed','Thu','Fri','Sat','Sun']; const data=(tr&&tr.data)||[12,19,7,15,9,13,11]; const ctx=c.getContext('2d'); new Chart(ctx,{ type:'line', data:{ labels:labels, datasets:[{ label:'Orders', data:data, borderColor:'#3b82f6', backgroundColor:'rgba(59,130,246,.15)', tension:.35, fill:true }]}, options:{ plugins:{legend:{display:false}}, scales:{ y:{ beginAtZero:true } } }); }); }
    // Chat local history load
    if ($('#aia-chat-list').length){ try{ const arr=JSON.parse(localStorage.getItem('aia_chat_history')||'[]'); arr.slice(-50).forEach(m=> { const $list=$('#aia-chat-list'); const who = m.role==='user'?'🧑':'🤖'; const line = $('<div>').text(m.content); $list.append($('<div>').append('<strong>'+who+'</strong>: ').append(line)); $list.scrollTop($list[0].scrollHeight); }); }catch(e){} }
  });

  $(document).on('submit', '#aia-chat-form', function(e){ e.preventDefault(); const $input=$('#aia-chat-input'); const msg=$input.val().trim(); if(!msg) return; const $list=$('#aia-chat-list'); const who='🧑'; const line=$('<div>').text(msg); $list.append($('<div>').append('<strong>'+who+'</strong>: ').append(line)); $list.scrollTop($list[0].scrollHeight); try{ const key='aia_chat_history'; const arr=JSON.parse(localStorage.getItem(key)||'[]'); arr.push({role:'user',content:msg,ts:Date.now()}); localStorage.setItem(key, JSON.stringify(arr).slice(-2000)); }catch(e){} $input.prop('disabled', true); post(AIA.rest + 'chat', { message: msg }).then(res=>{ const reply=(res && res.response) || 'OK'; const who='🤖'; const line=$('<div>').text(reply); $list.append($('<div>').append('<strong>'+who+'</strong>: ').append(line)); $list.scrollTop($list[0].scrollHeight); try{ const key='aia_chat_history'; const arr=JSON.parse(localStorage.getItem(key)||'[]'); arr.push({role:'assistant',content:reply,ts:Date.now()}); localStorage.setItem(key, JSON.stringify(arr).slice(-2000)); }catch(e){} }).catch(()=> { const who='🤖'; const line=$('<div>').text('Request failed'); $list.append($('<div>').append('<strong>'+who+'</strong>: ').append(line)); $list.scrollTop($list[0].scrollHeight); }).finally(()=> $input.prop('disabled', false)); });

  $(document).on('click', '#aia-chat-clear', function(){ localStorage.removeItem('aia_chat_history'); $('#aia-chat-list').empty(); });
  $(document).on('click', '#aia-chat-copy', function(){ const text = $('#aia-chat-list').text(); navigator.clipboard?.writeText(text); });
